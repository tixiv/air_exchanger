 ...
 */
extern const struct cmd_info _cmd_tbl_start[];
extern const struct cmd_info _cmd_tbl_end[];
...
static int common_prefix(const char *s, int n)
{
    ...
    const struct cmd_info *cmd = _cmd_tbl_start;
...
{
    const struct cmd_info *cmd1 = cmd_find(_cmd_tbl_start, line, cursor);
    if (cmd1 == NULL) {
        // no completion found
        //
        return cursor;
...
int cmd_exec(int argc, char *argv[])
{
    ...
    const struct cmd_info *cmd = cmd_find(_cmd_tbl_start, argv[0], INT_MAX);
